...
void removeNulls(struct concesionario *con, uint16_t n)
{
        int i;

        for (i = n; i < con->num_coches; i++) {
					  ...
					  uint32_t pos)
{
	...
	if (con->num_coches == 0) {
                con->flags &= ~(1 << CURSO_CONCESIONARIO_ATTR_COCHE);
				 ...
				 struct concesionario *con)
{
	...
	for (i = 0; i < con->num_coches; i++) {
		ret += curso_coche_snprintf(buf + ret, size - ret,
			con->garaje[i]);
